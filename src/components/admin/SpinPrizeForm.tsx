...
export function SpinPrizeForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      setError(data.error ?? "Gagal menambah hadiah spin.");
      return;
    }
    setName("");
    setWeight(10);
  ...
  return (
    <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-3">
      ...
      <div>
        <label className="block text-xs text-neutral-400 mb-1">Bobot (peluang relatif)</label>
        <input
          type="number"
          min={1}
          required
      ...
      <div>
        <label className="block text-xs text-neutral-400 mb-1">Nilai Poin Bonus (opsional)</label>
        <input
          type="number"
          min={0}
          value={pointsValue}
